...
function validateColumnOne() {
    ...
    // First Pass: Identify duplicates and validate codes
    rowIDs.forEach(function (row) {
        var escapedRind = row.rind.replace(/\./g, "\\."); // Escape dots in rind
        ...
        if (col1.length) {
            ...
            // Highlight and show error for invalid code
            if (!found && value) {
                highlightInvalid(col1); // Highlight invalid cell
                showInlineError(col1, "Codul nu face parte din clasificator");
            } else {
                removeHighlight(col1); // Remove invalid highlight if resolved
                removeInlineError(col1); // Remove inline error if valid
            }
