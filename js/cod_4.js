...
// Store validated codes to ignore pre-filled rows
const prefilledRowCodes = new Set();
...
$(document).ready(function () {
    // On page load, mark prefilled codes as validated
    markPrefilledCodes();
    // Add real-time validation
    setupRealTimeValidation();
});

// Function to mark prefilled codes when the page loads
function markPrefilledCodes() {
    rowIDs.forEach((row) => {
        const escapedRind = row.rind.replace(/\./g, "\\.");
        const col1 = $(`#14_340_${row.id}_${escapedRind}_1`); // COL1 identifier

        if (col1.length) {
            const value = col1.val().trim();
            if (value) {
                prefilledRowCodes.add(value); // Mark prefilled value
            }
        }
    });
}

// Function to set up real-time validation for new inputs
function setupRealTimeValidation() {
    $("input[data-col='1']").on("input", function () {
        ...
        // Skip validation for prefilled codes
        if (prefilledRowCodes.has(value)) {
            return;
        }
...
// Function to validate a single code in real-time
