    ...
    rowIDs.forEach(function (row) {
        ...
        if (col1.length) {
            ...
            if (enteredCodes.has(value)) {
                const duplicateRow = enteredCodes.get(value);

                // Highlight current cell and previously entered duplicate
                highlightDuplicate(col1);
                highlightDuplicate($(`#14_340_${duplicateRow.id}_${duplicateRow.rind.replace(/\./g, "\\.")}_1`));

                // Show inline error for duplicates
                showInlineError(col1, `Cod duplicat: ${value} (Randurile ${duplicateRow.rind} și ${row.rind})`);
            } else if (value) {
                // Add code to enteredCodes map with its row ID
                enteredCodes.set(value, row);
                removeHighlight(col1); // Remove duplicate highlight if resolved
                removeInlineError(col1); // Remove inline error if resolved
            }

            // Show error for invalid code
            if (!found && value) {
                showInlineError(col1, "Codul nu face parte din clasificator");
                highlightInvalid(col1); // Highlight invalid cells
            } else {
                removeHighlight(col1); // Remove invalid highlight if resolved
...
function highlightInvalid(col1) {
    col1.css({
